  
import { X } from "lucide-react";
import { useState } from "react";

interface DeliveryTimeProps {
    onClose?: () => void;
}

export default function DeliveryTime({ onClose }: DeliveryTimeProps) {
    const [loading, setLoading] = useState<boolean>(false);
    const [error, setError] = useState<string | null>(null);

    const [selectedItems, setSelectedItems] = useState<string[]>([]);
    const list = [
        { item: "Less than 30 mins" },
        { item: "Less than 45 mins" }
    ];

    const submitDelivery = () => {
        setLoading(true);
        setError(null);

        setTimeout(() => {
            setLoading(false);
            if (onClose) onClose();
        }, 2000);
    }

    return (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-[100] backdrop-blur-sm px-4">
            <div className="bg-white shadow-2xl rounded-2xl w-full max-w-[600px] flex flex-col relative animate-in fade-in zoom-in-95 duration-200">
                {/* Header */}
                <div className="flex justify-between items-center p-6 pb-4">
                    <h1 className="text-xl font-bold text-gray-900">Delivery Time</h1>
                    <button 
                        type="button"
                        onClick={onClose} 
                        className="p-1.5 border border-gray-200 rounded-full hover:bg-gray-100 hover:scale-105 transition-all shadow-sm bg-white cursor-pointer"
                        disabled={loading}
                    >
                        <X size={18} className="text-gray-500" strokeWidth={2.5} />
                    </button>
                </div>

                {/* Content */}
                <div className="flex flex-col gap-5 px-6 py-4 min-h-[300px]">
                    {list.map((i) => (
                        <label key={i.item} className="flex items-center gap-3 cursor-pointer group">
                            <input 
                                type="checkbox" 
                                checked={selectedItems.includes(i.item)}
                                onChange={(e) => {
                                    if (e.target.checked) {
                                        setSelectedItems(prev => [...prev, i.item]);
                                    } else {
                                        setSelectedItems(prev => prev.filter(item => item !== i.item));
                                    }
                                }}
                                className="w-5 h-5 rounded border-gray-400 text-[#ff5200] focus:ring-[#ff5200] accent-[#ff5200] cursor-pointer" 
                            />
                            <span className="text-gray-600 group-hover:text-gray-800 font-medium">
                                {i.item}
                            </span>
                        </label>
                    ))}
                </div>

                {/* Footer */}
                <div className="flex justify-between items-center px-6 py-4 border-t border-gray-100 bg-white rounded-b-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <button 
                        type="button"
                        onClick={() => setSelectedItems([])}
                        className="text-[#ff5200] font-bold bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer text-[15px] px-6 py-2"
                        disabled={loading}
                    >
                        Clear Filters
                    </button>
                    <button 
                        type="button"
                        onClick={submitDelivery}
                        disabled={loading}
                        className="bg-[#ff5200] text-white px-10 py-3 rounded-xl font-bold hover:bg-[#e04800] transition-colors shadow-md text-[15px] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center min-w-[120px]"
                    >
                        {loading ? 'Applying...' : 'Apply'}
                    </button>
                </div>
            </div>
        </div>
    );
}
