...
const DEFAULT_IMAGES = [
  ...
  {
    src: 'https://images.unsplash.com/photo-1499028344343-cd173ffc68a9?q=80&w=800&auto=format&fit=crop',
    alt: 'Gourmet burger'
  },
  {
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (!draggingRef.current && !openingRef.current && !inertiaRAF.current && !scrollLockedRef.current) {
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotateSpeed * dt * 0.05);
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      if (!mainR || !frameR || !viewerRef.current || tileR.width <= 0 || tileR.height <= 0) {
        openingRef.current = false;
        focusedElRef.current = null;
      ...
      if (wantsResize) {
        const onFirstEnd = (ev: { propertyName: string; }) => {
  ...
  const onTilePointerUp = useCallback(
      (    e: { pointerType: string; currentTarget: HTMLElement; }) => {
