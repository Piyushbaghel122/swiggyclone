...
var _chunkLU5KN3DHcjs = require('./chunk-LU5KN3DH.cjs');





var _chunk65NQFJTTcjs = require('./chunk-65NQFJTT.cjs');
...
function getSpecificationName(api) {
  return _chunkLU5KN3DHcjs.isSwagger.call(void 0, api) ? "Swagger" : "OpenAPI";
}
...
function fixOasRelativeServers(schema, filePath) {
  if (!schema || !_chunkLU5KN3DHcjs.isOpenAPI.call(void 0, schema) || !filePath || !filePath.startsWith("http:") && !filePath.startsWith("https:")) {
    return;
  }
  if (schema.servers) {
...
function repairSchema(schema, filePath) {
  if (_chunkLU5KN3DHcjs.isOpenAPI.call(void 0, schema)) {
...
function convertOptionsForParser(options) {
  const parserOptions = _chunk65NQFJTTcjs.getJsonSchemaRefParserDefaultOptions.call(void 0, );
  return {
    ...parserOptions,
    dereference: {
...
function validateSchema(api, options = {}, suppressedInstancePaths = []) {
  ...
  if (_chunkLU5KN3DHcjs.isSwagger.call(void 0, api)) {
    schema = _openapischemas.openapi.v2;
    ajv = initializeAjv();
  } else if (_chunkLU5KN3DHcjs.isOpenAPI32.call(void 0, api)) {
    throw new TypeError("OpenAPI 3.2 is currently unsupported.");
  } else if (_chunkLU5KN3DHcjs.isOpenAPI31.call(void 0, api)) {
...
var OpenAPISpecificationValidator = class extends SpecificationValidator {
  ...
  run() {
    ...
    if (_chunkLU5KN3DHcjs.isOpenAPI30.call(void 0, this.api)) {
...
async function parse(api, options) {
  ...
  const parser = new (0, _chunk65NQFJTTcjs.$RefParser)();
...
async function validate(api, options) {
  ...
  try {
    ...
    if (err instanceof _chunk65NQFJTTcjs.MissingPointerError) {
      return {
        valid: false,
        errors: [{ message: err.message }],
        warnings: [],
  ...
  if (!_chunkLU5KN3DHcjs.isSwagger.call(void 0, parser.schema) && !_chunkLU5KN3DHcjs.isOpenAPI.call(void 0, parser.schema)) {
    return {
      valid: false,
  ...
  if (_optionalChain([parser, 'access', _38 => _38.$refs, 'optionalAccess', _39 => _39.circular])) {
    if (circular$RefOption === true) {
      _chunk65NQFJTTcjs.dereference_default.call(void 0, parser, parserOptions);
