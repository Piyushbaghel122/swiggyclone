...
var _chunkZYPAVWY2cjs = require('./chunk-ZYPAVWY2.cjs');
...
function get(object, path) {
  ...
  const result = _optionalChain([pathArray, 'optionalAccess', _ => _.reduce, 'call', _2 => _2((prevObj, key) => _optionalChain([prevObj, 'optionalAccess', _3 => _3[key]]), object)]);
  return result;
}
function set(object, path, value) {
  const pathArray = Array.isArray(path) ? path : String(path).match(/([^[.\]])+/g);
  return _optionalChain([pathArray, 'optionalAccess', _4 => _4.reduce, 'call', _5 => _5((acc, key, i) => {
    if (acc[key] === void 0) {
      acc[key] = {};
    }
...
function getSubschemas(schema, api, opts, seenRefs = /* @__PURE__ */ new Set()) {
  ...
  if (subSchemaDataSize > 0) {
    ...
    if (resolvedSchema.properties && typeof resolvedSchema.properties === "object") {
      for (const [propName, propSchema] of Object.entries(resolvedSchema.properties)) {
        if (propSchema && typeof propSchema === "object") {
          let resolved;
          if (_types.isRef.call(void 0, propSchema)) {
            if (seenRefs.has(propSchema.$ref)) {
              continue;
            }
            seenRefs.add(propSchema.$ref);
            resolved = _utils.dereferenceRef.call(void 0, propSchema, api);
          } else {
            resolved = propSchema;
          }
    ...
    if ("items" in resolvedSchema && resolvedSchema.items !== void 0 && resolvedSchema.items !== true) {
      ...
      let resolved;
      if (_types.isRef.call(void 0, itemsSchema)) {
        if (!seenRefs.has(itemsSchema.$ref)) {
          seenRefs.add(itemsSchema.$ref);
          resolved = _utils.dereferenceRef.call(void 0, itemsSchema, api);
        }
      } else {
...
function collectSchemaObjectProperties(schema, api, seenRefs) {
  let node = schema;
  if (_types.isRef.call(void 0, node)) {
    if (seenRefs.has(node.$ref)) {
      return void 0;
    }
    seenRefs.add(node.$ref);
    const deref = _utils.dereferenceRef.call(void 0, node, api);
...
function parseJSONStringsInBodyWithSchema(obj, schema, api, seenRefs = /* @__PURE__ */ new Set()) {
  ...
  if (obj !== null && typeof obj === "object") {
    ...
    for (const [k, v] of Object.entries(obj)) {
      ...
      const schemaForValue = _nullishCoalesce(propSchema, () => ( additionalPropertiesSchema));
...
function stylizeValue(value, parameter) {
  ...
  if (parameter.content && (parameter.in === "query" || parameter.in === "header")) {
    ...
    let serialized;
    if (_utils.matchesMimeType.json(contentType)) {
...
function multipartBodyToFormatterParams(payload, oasMediaTypeObject, schema) {
  ...
  if (typeof payload === "object" && payload !== null) {
    return Object.keys(payload).map((key) => {
      ...
      const isObjectSchema = typeof propertySchema === "object" && propertySchema !== null && !_types.isRef.call(void 0, propertySchema);
...
function stringify(json) {
  return JSON.stringify(
    _removeundefinedobjects2.default.call(void 0, typeof json.RAW_BODY !== "undefined" ? json.RAW_BODY : json, {
  ...
  if (_optionalChain([requestBody, 'optionalAccess', _29 => _29.schema]) && Object.keys(requestBody.schema).length) {
    ...
    if (isFormUrlEncoded) {
      if (Object.keys(formData.formData || {}).length) {
        const cleanFormData = _removeundefinedobjects2.default.call(void 0, formData.formData, { preserveNullishArrays: true });
      ...
      const isMultipart = _utils.matchesMimeType.multipart(contentType);
      const isJSON = _utils.matchesMimeType.json(contentType);
      if (isMultipart || isJSON) {
        try {
          let cleanBody = _removeundefinedobjects2.default.call(void 0, formData.body, {
            preserveNullishArrays: true,
            preserveEmptyArray: true,
            preserveEmptyObject: {
              arrayItem: false,
