...
function formatEnumDescription(values) {
  return values.filter((value) => value !== void 0 && (typeof value !== "string" || value.trim() !== "")).map((value) => `\`${value}\``).join(" ");
}
function withoutGeneratedEnumDescription(schema) {
...
function toJSONSchema(data, opts) {
  ...
  function convertExamples(obj) {
    if ("example" in obj) {
      if (isPrimitive(obj.example)) {
        obj.examples = [obj.example];
      } else if (Array.isArray(obj.example)) {
        obj.examples = obj.example.filter((example) => isPrimitive(example));
      ...
      if (typeof obj.examples === "object" && obj.examples !== null && !Array.isArray(obj.examples)) {
        const examples = [];
        Object.entries(obj.examples).forEach(([name, example]) => {
      ...
      } else if (Array.isArray(obj.examples) && isPrimitive(obj.examples[0])) {
        reshapedExamples = true;
      }
      if (!reshapedExamples) {
        delete obj.examples;
    ...
    if ("allOf" in schema && Array.isArray(schema.allOf)) {
      ...
      try {
        ...
        for (const branch of branches) {
          ...
          if (fallback.title === void 0 && typeof branch.title === "string" && branch.title.length > 0) {
            fallback.title = branch.title;
          }
          if (fallback.description === void 0 && typeof branch.description === "string" && branch.description.length > 0) {
        ...
        if (Object.keys(mergedProperties).length > 0) {
          fallback.properties = mergedProperties;
          if (fallback.type === void 0) fallback.type = "object";
        }
        if (requiredSet.size > 0) {
          fallback.required = [...requiredSet];
        }
        schema = fallback;
      }
...
var RESERVED_HEADER_PARAMETERS = /* @__PURE__ */ new Set(["accept", "content-type"]);
function isReservedHeaderParameter(param, hasSecurity) {
...
function getParametersAsJSONSchema(operation, api, opts) {
      ...
      const properties = parameters.reduce((prev, current) => {
        ...
        if ("schema" in current) {
          ...
          if (contentKeys.length) {
            ...
            if (contentType && typeof current.content[contentType] === "object" && "schema" in current.content[contentType]) {
  ...
  return jsonSchema.map((group) => {
    if (group.schema && typeof group.schema === "object") {
      const usedSchemas = usedSchemasByGroup.get(group.type) ?? /* @__PURE__ */ new Map();
...
//# sourceMappingURL=chunk-KUSZJDZB.js.map