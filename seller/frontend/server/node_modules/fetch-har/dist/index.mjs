...
function getFileFromSuppliedFiles(filename, files) {
	if (files && filename in files) return files[filename];
	else if (files && decodeURIComponent(filename) in files) return files[decodeURIComponent(filename)];
	return false;
}
async function fetchHAR(har, opts = {}) {
	if (!har) throw new Error("Missing HAR definition");
	if (!har.log?.entries?.length) throw new Error("Missing log.entries array");
	if (!globalThis.Blob) try {
	...
	else if (typeof options.headers === "object" && !(options.headers instanceof Headers) && options.headers !== null) options.headers = new Headers(options.headers);
	const headers = options.headers;
	if ("headers" in request && request.headers.length) request.headers.forEach((header) => {
		try {
 ...
 /**
	...
	*/
	...
	} else headers.append("cookie", request.cookies.map((cookie) => `${encodeURIComponent(cookie.name)}=${encodeURIComponent(cookie.value)}`).join("; "));
	if ("postData" in request) {
		if (request.postData && "params" in request.postData) {
			if (!("mimeType" in request.postData)) request.postData.mimeType = "application/octet-stream";
					...
					request.postData.params?.forEach((param) => {
						if ("fileName" in param && param.fileName) {
							if (opts.files) {
								...
								if (fileContents) {
									if (isBuffer(fileContents)) {
										form.append(param.name, new File([new Uint8Array(fileContents)], param.fileName, { type: param.contentType || void 0 }), param.fileName);
										return;
							...
							if ("value" in param && param.value) {
								...
								if (parsed) paramBlob = new Blob([param.value], { type: parsed.contentType || param.contentType || void 0 });
								else paramBlob = new Blob([param.value], { type: param.contentType || void 0 });
			...
			if (opts.files) {
				const parsed = parse(request.postData.text);
				if (parsed) {
					if (parsed?.name) {
						const fileContents = getFileFromSuppliedFiles(parsed.name, opts.files);
						if (fileContents) {
							if (isBuffer(fileContents)) options.body = new Uint8Array(fileContents);
							else if (isFile(fileContents)) if (isBrowser()) options.body = fileContents;
							else {
								options.body = fileContents.stream();
								shouldSetDuplex = true;
								if (!headers.has("content-length")) headers.set("content-length", String(fileContents.size));
							}
						}
					}
				}
			}
			if (typeof options.body === "undefined") options.body = request.postData.text;
		}
		/**
		...
		*/
		if (shouldSetDuplex && !isBrowser()) options.duplex = "half";
	}
	let requestURL = url;
	if ("queryString" in request && request.queryString.length) {
		...
		if (urlObj.hash) {
			requestURL = `${requestURL.replace(urlObj.hash, "").split("?")[0]}${querystring ? `?${querystring}` : ""}`;
			requestURL += urlObj.hash;
		} else requestURL = `${requestURL.split("?")[0]}${querystring ? `?${querystring}` : ""}`;
	}
	if (opts.userAgent) headers.append("User-Agent", opts.userAgent);
